    ...
    for(l1 in 1:maxit){
      ...
      for(l2 in incomplete_variables){#do the imputation cycle
        ...
        if(fe$clID_varname != ""){
          ...
          my_data[, fe$clID_varname] <- sample_imp(my_data[, fe$clID_varname])
        ...
        if(tmp_type == "binary"){
          ...
          if(fe$clID_varname == "" || !(fe$clID_varname %in% names(my_data)) ||
            ...
            imp <- imp_binary_single(y_imp = my_data[, tmp_variable],
                             X_imp = tmp_X)

          }else{

            print("Currently the multilevel model.")
            imp <- imp_binary_multi(y_imp = my_data[, tmp_variable],
                                    X_imp = tmp_X,
                                    Z_imp = tmp_Z,
...
#             tmp_variable == fe$target_varname ||
            ...
            print("Currently the single level model.")
            imp <- imp_cont_single(y_imp = my_data[, tmp_variable],
                                  X_imp = tmp_X)

          }else{

            print("Currently the multilevel model.")
            imp <- imp_cont_multi(y_imp = my_data[, tmp_variable],
                             X_imp = tmp_X,
                             Z_imp = tmp_Z,
                             clID = my_data[, fe$clID_varname],
                             nitt = 3000,
...
#             tmp_variable == fe$target_varname ||
            ...
            print("Currently the single level model.")
            imp <- imp_semicont_single(y_imp = my_data[, tmp_variable],
                                      X_imp = tmp_X,
                                      heap = 0)
          }else{

            print("Currently the multilevel model.")
            imp <- imp_semicont_multi(y_imp = my_data[, tmp_variable],
                                  X_imp = tmp_X,
                                  Z_imp = tmp_Z,
                                  clID = my_data[, fe$clID_varname],
        ...
        if(tmp_type == "roundedcont"){
          ...
          #yet, we don't have a multilevel imputation for rounded incomes
            imp <- imp_roundedcont(y_imp = my_data[, tmp_variable],
                                   X_imp = tmp_X,
        ...
        if(tmp_type == "interval"){
          ...
          imp <- imp_interval(y_imp = my_data[, tmp_variable],
                                 X_imp = tmp_X)
...
#             tmp_variable == fe$target_varname ||
            ...
            print("Currently the single level model.")
            imp <- imp_count_single(y_imp = my_data[, tmp_variable],
                              X_imp = tmp_X)
            ...
            print("Currently the multilevel model.")
            imp <- imp_count_multi(y_imp = my_data[, tmp_variable],
                                   X_imp = tmp_X,
                                   Z_imp = tmp_Z,
                                   clID = my_data[, fe$clID_varname],
                                   nitt = nitt,
        ...
        if(tmp_type == "categorical"){
          ...
          if(fe$clID_varname == "" || !(fe$clID_varname %in% names(my_data)) ||
            ...
            print("Currently the single level model.")
            imp <- imp_cat_single(y_imp = my_data[, tmp_variable],
                                X_imp = tmp_X)

          }else{

            print("Currently the multilevel model.")
            imp <- imp_cat_multi(y_imp = as.factor(my_data[, tmp_variable]),
                                 X_imp = tmp_X,
                                 Z_imp = tmp_Z,
                                 clID = my_data[, fe$clID_varname],
                                 model_formula = model_formula,
 ...
 #            tmp_variable == fe$target_varname ||
            ...
            print("Currently the single level model.")
            imp <- imp_orderedcat_single(y_imp = my_data[, tmp_variable],
                                  X_imp = tmp_X)

          }else{

            print("Currently the multilevel model.")
            imp <- imp_orderedcat_multi(y_imp = as.factor(my_data[, tmp_variable]),
                                 X_imp = tmp_X,
                                 Z_imp = tmp_Z,
                                 clID = my_data[, fe$clID_varname],
                                 model_formula = model_formula,
