...
imp_cat_multi <- function(y_imp,
  ...
  colnames(rand_eff_sample) <- paste("effect for category", 1:(J-1))
  # zdraws has the following form:
  # c(cluster1 effect for category B, cluster 2 effect for categoriy B, cluster 3, 4, .., last cluster,
  #cluster 1 effect for category C, cluster 2, ...., last cluster,
  # ... cluster 1 effect for last category, ...., last cluster effect for last category)
