...
imp_cat_multi <- function(y_imp_multi,
  ...
  tmp0 <- data.frame(y_tmp = stats::rnorm(n), X_imp_multi_stand, cl.id = clID)
  tmp0_formula <- stats::formula(paste("y_tmp ~ 0 + ", paste(colnames(X_imp_multi_stand), collapse = "+")))

  X_model_matrix <- stats::model.matrix(tmp0_formula, data = tmp0)
  ...
  for (j in 1:M){
    ...
    fix_eff_sample <- xdraws[select.record[j], ]
    rand_eff_sample <- zdraws[select.record[j], ]
